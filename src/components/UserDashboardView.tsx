...
export function UserDashboardView({ session, privateKey }: UserDashboardViewProps) {
  ...
  }, [selectedContact, session.user, onlineUsers]);
          ...
          <div className="p-6 mt-auto border-t border-white/5 bg-white/[0.01]">
            <div className="flex items-center gap-4">
              <div className="relative">
                <motion.div 
                  animate={partnerPresence.isTyping ? { y: [0, -8, 0] } : {}}
                  transition={{ duration: 0.6, repeat: Infinity, ease: "easeInOut" }}
                  className={`w-3 h-3 rounded-full shadow-[0_0_15px_rgba(59,130,246,0.5)] transition-all duration-500 ${
                    partnerPresence.isTyping || partnerPresence.isInChat ? 'bg-blue-500 scale-110' : 'bg-zinc-700'
                  } ${partnerPresence.isInChat ? 'animate-pulse' : ''}`}
                />
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-black uppercase italic tracking-widest text-white/80 truncate">
                  {selectedContact.username}
                </p>
                <p className="text-[8px] font-bold uppercase tracking-[0.2em] text-blue-400/80">
                  {partnerPresence.isTyping ? 'Typing Signal...' : partnerPresence.isInChat ? 'Inside Channel' : partnerPresence.isOnline ? 'Active Pulse' : 'Signal Lost'}
                </p>
              </div>
            </div>
          </div>
                      ...
                      {recentChats.map(chat => (
                        <button key={chat.id} onClick={() => { setSelectedContact(chat); setActiveView("chat"); }} className="w-full flex items-center gap-4 p-4 hover:bg-white/5 rounded-2xl transition-all">
                          ...
                          <div className="flex-1 text-left">
                            ...
                            <p className={`text-[8px] font-bold uppercase tracking-widest ${onlineUsers.has(chat.id) ? 'text-emerald-500' : 'text-zinc-600'}`}>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {profiles.map(p => (
                        <div key={p.id} onClick={() => { setSelectedContact(p); setActiveView("chat"); }} className="p-4 bg-white/[0.02] border border-white/5 rounded-2xl flex flex-col items-center gap-3 cursor-pointer hover:border-indigo-500/30">
                          ...
                          <p className={`text-[8px] font-bold uppercase tracking-widest ${onlineUsers.has(p.id) ? 'text-emerald-500' : 'text-zinc-600'}`}>
