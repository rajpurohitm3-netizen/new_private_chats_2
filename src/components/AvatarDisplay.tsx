...
export function AvatarDisplay({ profile, className = "h-12 w-12", isOnline }: { profile: any; className?: string; isOnline?: boolean }) {
  if (!profile?.avatar_config) {
    return (
      <div className="relative inline-block">
        ...
        {isOnline !== undefined && (
          <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-[#030303] ${isOnline ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.8)]' : 'bg-zinc-600'}`} />
        )}
      </div>
    );
  }
